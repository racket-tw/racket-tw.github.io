...
事實上我們要重複幾行 @code{(displayln "hello, world")} 都無所謂，這些都是程式，只是沒有解決我們可能關心的問題而已。我們關心的是問題有沒有被解決，寫程式只是其中一種辦法而不是最重要的部分。而程式具備的重要特性之一，不單是可以執行，而是可以被閱讀。一個方法可以通過被閱讀理解後傳承，稍作修改就能解決新的問題，或是在這之上解決更複雜的問題，不需要反覆手動執行一次流程，才是程式的價值所在。
